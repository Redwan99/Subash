    ...
    <>
      ...
      <div className="lg:hidden mb-4">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/10 transition-colors w-full sm:w-auto"
        >
          <SlidersHorizontal className="w-4 h-4" />
          Filters
          {activeFilterCount > 0 && (
            <span className="ml-auto sm:ml-1 text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-brand-500 text-white">
              {activeFilterCount}
            </span>
          )}
        </button>
        {mobileFiltersOpen && (
          <div className="mt-3 p-5 rounded-2xl bg-gray-50 dark:bg-[#0a0a0a] border border-gray-200 dark:border-white/10 shadow-xl animate-fade-in-up">
            {filterContent}
          </div>
        )}
      </div>
      ...
      {isPending ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 sm:gap-6 w-full contain-paint">
          {results.map(perfume => (
