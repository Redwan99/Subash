...
export async function searchEncyclopedia(filters: {
  ...
  try {
    ...
    // Default page size 40; pass take=0 to explicitly fetch all (not recommended)
    const take = typeof filters.take === "number" ? (filters.take === 0 ? undefined : filters.take) : 40;
