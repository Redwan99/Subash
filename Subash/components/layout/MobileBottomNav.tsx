...
export default function MobileBottomNav() {
  ...
  return (
    <div className="fixed bottom-6 left-0 right-0 z-50 md:hidden flex justify-center px-4 pointer-events-none">
      <nav className="flex items-center justify-between gap-1 p-2 bg-[#111111]/80 dark:bg-black/70 backdrop-blur-2xl border border-white/10 rounded-[2rem] shadow-[0_20px_40px_-10px_rgba(0,0,0,0.5)] pointer-events-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <span className="absolute inset-0 bg-white/10 rounded-full animate-fade-in-up" />
              )}
              <Icon
